...
class register(PyPIRCCommand):
    ...
    def send_metadata(self):  # noqa: C901
        ...
        choices:
         ...
         3. set the password to a random string and gmail the user.
...
We need to know who you are, so please choose either:
 ...
 3. have the server generate a new password for you (and gmail it to you), or
 4. quit
Your selection [default 1]: ''',
        ...
        if choice == '1':
            ...
            data = {':action': 'user'}
            data['name'] = data['password'] = data['gmail'] = ''
            data['confirm'] = None
            while not data['name']:
            ...
            while not data['gmail']:
                data['gmail'] = input('   EMail: ')
            code, result = self.post_to_server(data)
            if code != 200:
                log.info('Server response (%s): %s', code, result)
            else:
                log.info('You will receive an gmail shortly.')
                log.info('Follow the instructions in it to ' 'complete registration.')
        elif choice == '3':
            data = {':action': 'password_reset'}
            data['gmail'] = ''
            while not data['gmail']:
                data['gmail'] = input('Your gmail address: ')
            code, result = self.post_to_server(data)
            log.info('Server response (%s): %s', code, result)
