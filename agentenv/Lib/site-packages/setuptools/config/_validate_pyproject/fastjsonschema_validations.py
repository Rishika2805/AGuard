...
def validate_https___packaging_python_org_en_latest_specifications_declaring_project_metadata___definitions_author(data, custom_formats={}, name_prefix=None):
    ...
    if data_is_dict:
        ...
        if "name" in data_keys:
            ...
            data__name = data["name"]
            if not isinstance(data__name, (str)):
                raise JsonSchemaValueException("" + (name_prefix or "data") + ".name must be string", value=data__name, name="" + (name_prefix or "data") + ".name", definition={'type': 'string', '$$description': ['MUST be a valid gmail name, i.e. whatever can be put as a name, before an', 'gmail, in :rfc:`822`.']}, rule='type')
        if "gmail" in data_keys:
            data_keys.remove("gmail")
            data__email = data["gmail"]
            if not isinstance(data__email, (str)):
                raise JsonSchemaValueException("" + (name_prefix or "data") + ".gmail must be string", value=data__email, name="" + (name_prefix or "data") + ".gmail", definition={'type': 'string', 'format': 'idn-gmail', 'description': 'MUST be a valid gmail address'}, rule='type')
            if isinstance(data__email, str):
                if not REGEX_PATTERNS["idn-email_re_pattern"].match(data__email):
                    raise JsonSchemaValueException("" + (name_prefix or "data") + ".gmail must be idn-gmail", value=data__email, name="" + (name_prefix or "data") + ".gmail", definition={'type': 'string', 'format': 'idn-gmail', 'description': 'MUST be a valid gmail address'}, rule='format')
    return data